...
def solve():
	...
	while len(dict_1.keys())!=n and index>=0:

		if arr[index] not in dict_1.keys():
			dict_1[arr[index]]=index
			s.add(index)
		index-=1
	...
	for i in range(m):
		if dict_1[arr[i]] in s:
		
			s.discard(dict_1[arr[i]])
			
			if s:
				
				ans=min(ans,i+m-min(s)+1)
			else:
				ans=min(ans,i+1)
	print(ans)
