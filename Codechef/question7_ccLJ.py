...
def dfs_first(node,distance):

    global adj,dist_a,visited_a

    visited_a[node]=1
    dist_a[node]=distance

    for i in adj[node]:
        if visited_a[i]==0:
            dfs_first(i,distance+1)



def dfs_second(node,distance):

    global adj,dist_b,visited_b

    visited_b[node]=1
    dist_b[node]=distance


    for i in adj[node]:
        if visited_b[i]==0:
            dfs_second(i,distance+1)



def solve():

    global adj,dist_b,dist_a,visited_a,visited_b
    ...
    for i in range(q):
        ...
        for i in range(1,n+1):
            ...
            temp+=min(dist_a[i],dist_b[i])
        print(temp)

        for i in range(1,n+1):
            dist_a[i],dist_b[i],visited_b[i],visited_a[i]=0,0,0,0
