...
def solve():
    ...
    for i in range(m):
        ...
        first = bisect_left(arr, a)
        
        ans =  float("inf")
        # print("inwex",first)
        if first!=n and first!=0:
            second=max(0,a-arr[first-1])+max(0,b-(summ-arr[first-1]))

            third=max(0,a-arr[first])+max(0,b-(summ-arr[first]))

            print(min(second,third))
            
        
        else:

            if first==n:
                print(a-arr[first-1]+max(0,b-(summ-arr[first-1])))
                

            
            else:
                print(max(0,b-(summ-arr[first])))
